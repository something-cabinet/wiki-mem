...
Wiki pages are stored as `.wm/wiki/<subdir>/<slug>.md` and accessible via `wm:page.get({"id": "<subdir>/<slug>"})`.
...
```json
wm:search.query({ "query": "<topic>", "type": "all", "mode": "keyword" })
wm:memory.list({ "category": "pattern", "tag": "<domain>" })
```
...
```json
wm:memory.add({ "title": "<Pattern Name>",
