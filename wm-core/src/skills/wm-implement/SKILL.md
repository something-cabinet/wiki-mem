...
```json
wm:task.update({ "taskId": "$ARGUMENTS", "status": "in-progress", "appendNotes": "Reopened: <reason>" })
wm:time.start({ "taskId": "$ARGUMENTS" })
```

## Step 2: Set Status

```json
wm:task.update({ "taskId": "$ARGUMENTS", "status": "in-progress" })
wm:time.start({ "taskId": "$ARGUMENTS" })
```
...
```json
wm:task.update({ "taskId": "$ARGUMENTS", "checkAc": [1], "appendNotes": "Done: brief description" })
```
...
```json
wm:task.update({ "taskId": "$ARGUMENTS", "addAc": ["New requirement"], "appendNotes": "Scope: added per user" })
```
...
```json
wm:validate.check({ "entity": "$ARGUMENTS" })
```
...
```json
wm:time.stop({ "taskId": "$ARGUMENTS" })
wm:task.update({ "taskId": "$ARGUMENTS", "status": "done" })
```
...
```json
wm:task.list({ "spec": "<spec-path-from-task>" })
```
