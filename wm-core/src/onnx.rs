// ─── OnnxEmbedder — ONNX Runtime powered embedding ──────────
// Feature-gated behind "embed". Only compiles with ort + tokenizers.
...
use crate::embed::{EmbedError, EmbedVector, Embedder};

/// ONNX Runtime-based embedder using a BERT-style model (e.g., bge-small-en-v1.5).
...
impl OnnxEmbedder {
    /// Load model from disk. Returns Ok(None) if model directory is missing
    /// — caller falls back to BM25-only.
    pub fn load(model_dir: &Path, model_name: &str) -> Result<Option<Self>, EmbedError> {
        ...
        // Bump the ONNX environment initialization
        ...
        // Infer dimension from model output — default to 384 for BERT models
        // (output shape is read from first inference batch at runtime)
        let dim = 384;
...
impl Embedder for OnnxEmbedder {
    ...
    fn embed_batch(&self, texts: &[&str]) -> Result<Vec<EmbedVector>, EmbedError> {
        ...
        // Encode with tokenizer — encode_batch takes Vec<&str>
        let encoding = self
            .tokenizer
        ...
        // Create tensors using owned data (Vec<i64> for data, Vec<i64> for shape)
        ...
        // Run inference — keep session guard alive for the duration
        ...
        // Extract output tensor — shape: [batch, seq_len, dim]
        ...
        // CLS-pool: take position 0 for each batch item from flat tensor data
...
const MODEL_REGISTRY: &[ModelEntry] = &[
    ModelEntry {
        ...
        // Replace with real SHA-256 hash from HuggingFace model card
...
/// Download a model and its tokenizer. Returns the model directory path.
pub fn download_model(model_name: &str, models_dir: &Path) -> Result<PathBuf, EmbedError> {
    ...
    // H12: If vectors.bin from a different model exists, delete it
    ...
    if vectors_path.exists() {
        // Read the model_name from the vectors.bin header
