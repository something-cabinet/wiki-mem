...
impl JsonRpcResponse {
    ...
    pub fn error(id: Option<Value>, err: &ToolError) -> Self {
        Self { jsonrpc: "2.0".into(), result: None, error: Some(err.to_json()), id }
    }
...
impl ToolRegistry {
    ...
    pub fn list_tools(&self) -> Value {
        serde_json::json!(self.handlers.iter().map(|(name, _)| {
...
pub async fn run_transport(registry: Arc<ToolRegistry>) -> Result<(), anyhow::Error> {
    ...
    for line in reader.lines() {
        ...
        let response = match request.method.as_str() {
            "initialize" => {
                JsonRpcResponse::success(request.id, make_initialize_response())
            }
            "tools/list" => {
                JsonRpcResponse::success(request.id, registry.list_tools())
            }
            "tools/call" => {
                ...
                match result {
                    ...
                    Err(panic_info) => {
                        ...
                        JsonRpcResponse::error(request.id, &ToolError::internal("Internal server error"))
                    }
                }
            }
