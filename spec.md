...
- D12: 9 typed edge types (extends, implements, example_of, part_of, relates_to, supersedes, depends_on, answers, references + custom). Custom edge types require explicit registration in config.json (validated at graph build time). Pruned (2026-07-20): supports, contradicts, required_by, questions, similar_to, causes, mitigates — zero usage, overlapping semantics.
...
- **FR-9:** Task status queryable via `search.query(type="task", status="in-progress")` and graph queryable via `graph.neighbors(task_id)` to find `depends_on` edges.
...
- **FR-27:** Edge-type priority for context assembly (when no query): extends(10), implements(9), supersedes(8), part_of(8), example_of(6), depends_on(5), answers(5), references(1), relates_to(0). Pruned types (supports, contradicts, required_by, questions, similar_to, causes, mitigates) removed 2026-07-20.
...
| `answers` | 5 | Response to question | Resolution |
...
**Day 1:** Agent creates `concept:oauth2-flow` and `decision:use-pkce` with edge `references → pattern:pkce-extension`
**Day 2:** Agent creates `pattern:client-secret-basic` with edge `relates_to → pattern:pkce-extension`
