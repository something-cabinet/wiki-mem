...
declare const browser: {
  mockIPC: (cmd: string, handler: (args: unknown) => Promise<unknown>) => Promise<void>;
};
...
export async function registerTauriMocks(registry: MockRegistry): Promise<void> {
  for (const [cmd, mapping] of Object.entries(CMD_MAP)) {
    await browser.mockIPC(cmd, async (args: unknown) => {
      const query = extractQuery(cmd, args);
      const matched = registry.find(mapping.method, mapping.urlPath, query);
      if (!matched) {
        console.warn(`[wm-mock] No stub for IPC "${cmd}" → ${mapping.method} ${mapping.urlPath}`);
        return undefined;
      }
...
function extractQuery(cmd: string, args: unknown): Record<string, string> | undefined {
