...
pub(crate) fn get_or_create_parser(ext: &str) -> Option<&'static std::sync::Mutex<Parser>> {
    ...
    Some(cell.get_or_init(|| {
        ...
        parser
            ...
            .expect("tree-sitter language setup");
...
pub(crate) fn parse_source(source: &str, ext: &str) -> Result<tree_sitter::Tree, String> {
    let parser_mutex =
        get_or_create_parser(ext).ok_or_else(|| format!("Unsupported extension: {}", ext))?;
    let mut parser = parser_mutex
        .lock()
        .map_err(|e| format!("Parser lock error: {}", e))?;
    parser
        .parse(source, None)
        .ok_or_else(|| "Failed to parse source".into())
}
    ...
    while let Some(match_) = query_matches.next() {
        for capture in match_.captures {
            if capture.index == name_index {
                ...
                let start_line = range.start_point.row.wrapping_add(1);
...
pub(crate) fn get_line_at_offset(source: &str, offset: usize) -> &str {
    ...
    let line_start = before.rfind('\n').map(|i| i.wrapping_add(1)).unwrap_or(0);
