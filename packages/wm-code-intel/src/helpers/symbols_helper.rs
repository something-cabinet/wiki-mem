use crate::helpers::parser_helper::{compile_query, get_line_at_offset, parse_source, run_query};
...
fn for_language(
    ...
    for (query_str, kind) in queries {
        if let Ok(cq) = compile_query(lang, query_str, kind) {
            for (name, line, col, _sb, _eb) in run_query(
                &cq.query,
                cq.name_index,
                tree.root_node(),
                source.as_bytes(),
...
pub(crate) fn for_rust(source: &str, file: &str, language: &str) -> Vec<CodeIntelSymbol> {
    for_language(
        ...
        &[
            ...
            (
                r"(impl_item trait: [(type_identifier) (qualified_type)]? @name)",
                "impl",
            ),
        ],
    )
...
pub(crate) fn for_typescript(
    ...
    for_language(
        ...
        &[
            ...
            (
                r"(method_definition name: (property_identifier) @name)",
                "method",
            ),
            (
                r"(class_declaration name: (type_identifier) @name)",
                "class",
            ),
            (
                r"(interface_declaration name: (type_identifier) @name)",
                "interface",
            ),
            (
                r"(type_alias_declaration name: (type_identifier) @name)",
                "type",
            ),
            (r"(enum_declaration name: (identifier) @name)", "enum"),
            (
                r"(abstract_method_signature name: (property_identifier) @name)",
                "method",
            ),
            (
...
pub(crate) fn for_python(source: &str, file: &str, language: &str) -> Vec<CodeIntelSymbol> {
    for_language(
        ...
        &[
            ...
            (
                r"(decorated_definition (function_definition name: (identifier) @name))",
                "function",
            ),
            (
                r"(decorated_definition (class_definition name: (identifier) @name))",
                "class",
            ),
        ],
    )
