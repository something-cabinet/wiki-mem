...
acceptance_criteria:
  - text: "tools/list returns all registered tools, each with a non-empty description and non-empty input schema (AC-2)"
  - text: "initialize response advertises tools capability without listChanged and no list_changed notification is ever sent (AC-11)"
  - text: "cargo check -p wm-cli -p wm-core -p wm-server passes clean (AC-7)"
---
