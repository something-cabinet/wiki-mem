...
acceptance_criteria:
  - text: "WebGL canvas renders the 384 loaded nodes and 25 edges — pixel readback no longer returns [0,0,0,0]"
  - text: "Root cause identified among the hypothesized causes (regl draw failure, out-of-viewport coordinates, context loss, regl version, fitToView) and fixed"
  - text: "fitToView() places nodes within the visible viewport (no all-at-origin transform)"
---
