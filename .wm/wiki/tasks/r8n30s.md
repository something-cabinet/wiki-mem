...
acceptance_criteria:
  - text: "Cargo workspace with wm-core and wm-cli crates compiles (cargo build)"
  - text: "MCP JSON-RPC transport (initialize/tools/list/tools/call) reads from stdin and writes to stdout, with ToolError codes REQUIRED_FIELD/NOT_FOUND/NO_PROJECT/INVALID_ACTION and hints"
  - text: "Project auto-detection walks up from cwd or WM_PROJECT, config.json is loaded, catch_unwind wraps handlers, and SIGINT/SIGTERM triggers flush + persist + clean exit"
---
...
📚 Extracted to @wiki/concepts/learning-wiki-mem-graph-architecture (patterns: ArcSwap lock-free, two-tier staleness; decisions: ArcSwap over RwLock; failures: flat binary redesigns, DashMap id_index)
