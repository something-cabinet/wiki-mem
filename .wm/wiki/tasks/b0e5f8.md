...
id: wiki:tasks:b0e5f8
type: task
relates_to:
- type: implements
  target: wiki:specs:webgl-graph-rendering
acceptance_criteria:
- text: compute_layout no longer blocks the Tauri async runtime — it yields via tokio::task::yield_now or runs on spawn_blocking
- text: Other Tauri commands are not delayed during layout computation
- text: Progressive position events (graph-coarse, graph-refine, graph-settled) still fire correctly
status: cancelled
---
