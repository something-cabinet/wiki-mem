...
acceptance_criteria:
- text: start_layout Tauri IPC command accepts nodes + edges and runs the fjadra force simulation on a background thread (tokio::spawn)
- text: Positions stream via app_handle.emit('graph-positions', batch) in a hybrid two-phase (coarse clusters then refined batches) and cancel when the event listener drops
- text: Angular WebglGraphRenderer consumes Float32Array position updates, with the Canvas 2D fallback retained for graphs under 500 nodes
---
