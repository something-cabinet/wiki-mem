...
priority: low
acceptance_criteria:
  - text: "apps/wm-web/src-tauri/src/lib.rs run() reverted to the let mut builder pattern (or kept with a comment documenting the trade-off)"
  - text: "Chosen pattern documented; adding another cfg-dependent builder call no longer triggers a 'consumed value' compile error"
---
