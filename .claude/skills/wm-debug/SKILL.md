...
```json
wm_search.search({"query": "<error message>", "type": "doc", "tag": "learning"})
wm_search.search({"query": "<error message>", "type": "memory"})
wm_search.search({"query": "<error pattern>", "type": "doc"})
```
...
```json
wm_project.status()
wm_docs.get({"path": "<relevant-doc>", "smart": true})
wm_code.graph({"query": "<affected-module>"})
```
...
```json
wm_tasks.create({"title": "Fix: <root cause summary>",
...
```json
wm_memory.add({"title": "Debug: <error pattern>",
...
```json
wm_search.search({"query": "<failure domain>", "type": "doc", "tag": "learning"})
```

**If existing learning doc found — update it:**
```json
wm_docs.update({"path": "<existing-path>",
  "appendContent": "\n\n## <Date> — <Classification>\n\n**Root cause:** <sentence>\n**Signal:** <how to recognize>\n**Fix:** <what resolves it>"})
```

**If no existing doc — create new:**
```json
wm_docs.create({"title": "Learning: <domain> — <pattern>",
  "folder": "learnings",
...
```json
wm_docs.update({"path": "<learning-path>",
