...
```json
wm_docs.get({"path": "specs/<spec-path>", "smart": true})
```
...
```json
wm_search.resolve({"ref": "@doc/specs/<spec-path>{implements}", "direction": "inbound", "entityTypes": "task"})
```
...
```json
wm_tasks.list({"spec": "specs/<spec-path>"})
```
