...
```json
wm_templates.get({"name": "<template-name>"})
```
...
```json
wm_docs.get({"path": "<doc-path>", "smart": true})
```
...
```json
// Dry run first
wm_templates.run({"name": "<template-name>",
...
```json
wm_templates.create({"name": "<template-name>",
...
```json
wm_templates.run({"name": "new_component",
  "variables": {
