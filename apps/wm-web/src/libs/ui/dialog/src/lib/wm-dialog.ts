...
@Component({
  ...
  template: `
    <brn-dialog [state]="isOpen() ? 'open' : 'closed'" (stateChanged)="close.emit()">
      <brn-dialog-overlay
        (click)="close.emit()"
        class="fixed inset-0 bg-black/60 backdrop-blur-[2px] z-40 transition-all duration-200 ease-out data-[state=closed]:opacity-0 data-[state=open]:opacity-100"
      />
      <div
        brnDialogContent
