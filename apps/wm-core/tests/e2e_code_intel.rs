...
"#,
    ...
    assert_eq!(stats.symbols_indexed, 4, "greet, User, Status, helper");
    assert_eq!(stats.deps_indexed, 0);

    assert_eq!(
        db.query_symbols(None, None, None, None, None, None).unwrap().len(),
        4
    );
...
"#,
    ...
    create_source(&root, "src/models.rs", r#"use serde::{Serialize, Deserialize};"#);
    ...
    assert!(
        rev.iter().any(|v| v.get("file").and_then(|f| f.as_str()).is_some_and(|f| f.contains("engine"))),
        "reverse deps should include engine.rs"
    );
}
...
fn e2e_code_incremental() {
    ...
    assert_eq!(third.symbols_indexed, 2, "file changed — both symbols re-indexed");
...
fn e2e_code_multi_language() {
    ...
    for (lang, expected) in [
        ...
        assert_eq!(
            db.query_symbols(None, None, None, None, Some(lang), None).unwrap().len(),
            expected
        );
    }
    assert!(
        !db.query_symbols(None, None, None, None, Some("go"), None).unwrap().is_empty()
    );
}
