...
fn memory_as_wiki_page() {
    ...
    run_cli_with_stdin(
        &root,
        &["page", "create", "memory/e2e-memory", "E2E Memory", "--page-type", "memory"],
    ...
    let mem_file = root.join(".wm/wiki/memory/e2e-memory.md");
    assert!(mem_file.exists(), "memory page file should exist at {}", mem_file.display());
}
