//! Code-edge access for graph tools.
//!
...
///
/// Reads pre-materialized resolved edges from the DB when available (resolution
/// runs at index time, not per query). Falls back to on-the-fly resolution only
/// when the `resolved_edges` table is empty (first-time use before
/// `wm index code` writes them).
///
...
/// from whatever `wm index code` last wrote.
