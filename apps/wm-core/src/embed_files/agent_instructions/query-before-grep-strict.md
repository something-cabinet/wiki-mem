...
In this project raw file reads (`read`/`grep`/`bash`) are gated by a
permission rule: every raw read requires explicit approval. Issue a
`wm_graph.*` or `wm_search.*` query BEFORE the first raw read — the approval
gate is the enforcement backstop, not the primary path. If a raw read prompts
for approval before you have queried the graph, stop, query `wm_graph` /
`wm_search` first, then retry.
